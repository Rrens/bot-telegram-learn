...
def generate_text_report(response):
    ...
    today = today.strftime("%d-%m-%Y %H:%M:%S WIB")
    text = ''
    text += '🪧 *Weekly Report "Lapor Kendala" IOMS Last 7 Days*\n'
    ...
    try:
        ...
        for data in data_list_swfm:
            ...
            split_elements = problem_title.split(' ')
            if len(split_elements) != len(set(split_elements)):
                # print('salah')
                unique_elements = list(OrderedDict.fromkeys(split_elements))
                cleaned_problem_title = ' '.join(unique_elements)
            else:
                cleaned_problem_title = problem_title
            
            # print(cleaned_problem_title[0])
                
            if cleaned_problem_title not in unique_titles:
                unique_titles.append(cleaned_problem_title)
                
                if 'open' in data:
                    # print(cleaned_problem_title)
                    count_open_swfm.append(cleaned_problem_title)
                    x.append(cleaned_problem_title)
                if 'closed' in data:
                    count_closed_swfm.append(cleaned_problem_title)
                    x.append(cleaned_problem_title)
        # text = data_list_swfm
